 ...
 */
public class GetFlowSpeedTemporary extends UDAF {

  private static final Log LOGGER = LogFactory.getLog(GetFlowSpeedTemporary.class.getName());
  ...
  public static class PartialResult {
    private ArrayList<Double> sumFlow_list;
...
//            String flowType;
  ...
  public static class GetSpeedUDAFEvaluator implements UDAFEvaluator {
    ...
    private void splitPieces2(double sum_flow, String begin_time, String end_time) {
      if (pr.count == 0) {
        pr.sumFlow_list.add(sum_flow);
        pr.beginTime_list.add(begin_time);
        pr.endTime_list.add(end_time);
        setCount(pr);
      } else {
        double sum_sf = sum_flow;
        String min_bt = begin_time;
        String max_et = end_time;
        ArrayList<Integer> indexToRmList = new ArrayList<>();
        for (int i = 0; i < pr.count; i++) {
          double sf = pr.sumFlow_list.get(i);
        ...
        for (int j = 0; j < indexToRmList.size(); j++) {
          ...
          int rmIndex = indexToRmList.get(j) - j;
//                    LOGGER.info("index to remove: " + rmIndex);
          // Remove element using index with type int, not Integer!
          pr.sumFlow_list.remove(rmIndex);
          pr.beginTime_list.remove(rmIndex);
          pr.endTime_list.remove(rmIndex);
        }
        indexToRmList.clear();
...
//            long sum_flow_long;
      double sum_flow_double;
      try {
        sum_flow_double = Double.parseDouble(sum_flow);
      } catch (NumberFormatException e) {
        LOGGER.error("sum_flow '" + sum_flow + "' is not a number.");
        return true;
      }
      if (pr == null) {
        pr = new PartialResult();
      ...
      if (flow_type != null && !flow_type.equals("") && flow_type.length() == 3) {
        if (flow_type.startsWith("4") || (STREAMING_FLOW_TYPE_LIST.contains(flow_type) && STREAMING_CONTENT_TYPE_LIST.contains(contentType))) {
          // STREAMING
          if (sum_flow_double < 480 * 1024) {
            return true;
          }
        } else if (flow_type.startsWith("2")) {
          // HTTP
          if (sum_flow_double < 200 * 1024) {
            return true;
          }
        } else {
          // OTHERS
          if (sum_flow_double < 200 * 1024 && sum_flow_double / DateFormat.dateMinus(end_time, begin_time) < 50 * 1024 / 8) {
            return true;
          }
        }
      } else {
...
//                LOGGER.info("splitPieces2(sum_flow_long,begin_time,end_time): " + sum_flow_long + "," + begin_time + "," + end_time);
        splitPieces2(sum_flow_double, begin_time, end_time);
//                printLog();
      }
      return true;
    }
...
//            LOGGER.info("****splitPieces******");
      ...
      ArrayList<Double> sf_tmp = new ArrayList<>();
