 ...
 */
public class GetFlowSpeed extends UDAF {

  private static final Log LOGGER = LogFactory.getLog(GetFlowSpeed.class.getName());
  ...
  public static class PartialResult {
    private ArrayList<Long> sumFlow_list;
...
//            String flowType;
  ...
  public static class GetSpeedUDAFEvaluator implements UDAFEvaluator {
    ...
    private void splitPieces2(long sum_flow, String begin_time, String end_time) {
      if (pr.count == 0) {
...
//            Integer sum_flow_int;
      long sum_flow_long;
      try {
        sum_flow_long = Long.parseLong(sum_flow);
      } catch (NumberFormatException e) {
        LOGGER.error("sum_flow '" + sum_flow + "' is not a int number.");
        return true;
      }
      if (pr == null) {
        pr = new PartialResult();
      ...
      if (flow_type != null && !flow_type.equals("") && flow_type.length() == 3) {
        if (flow_type.startsWith("4") || (STREAMING_FLOW_TYPE_LIST.contains(flow_type) && STREAMING_CONTENT_TYPE_LIST.contains(contentType))) {
          // STREAMING
          if (sum_flow_long < 480 * 1024) {
            return true;
          }
        } else if (flow_type.startsWith("2")) {
          // HTTP
          if (sum_flow_long < 200 * 1024) {
            return true;
          }
        } else {
          // OTHERS
          if (sum_flow_long < 200 * 1024 && sum_flow_long / DateFormat.dateMinus(end_time, begin_time) < 50 * 1024) {
            return true;
          }
        }
      }
      if (!sum_flow.equals("") && begin_time != null && !begin_time.equals("") && end_time != null && !end_time.equals("")) {
        ...
        splitPieces2(sum_flow_long, begin_time, end_time);
        printLog();
      }
      return true;
    }
...
//            LOGGER.info("****splitPieces******");
      ...
      ArrayList<Long> sf_tmp = new ArrayList<>();
